...
{
	ComPtr<IDXGIAdapter4> adapter = Mox::GetMainAdapter(false);

	m_D3d12Device = Mox::CreateDevice(adapter);

#if _DEBUG
	Mox::ThrowIfFailed(DXGIGetDebugInterface1(0, IID_PPV_ARGS(&m_DxgiDebug)));
