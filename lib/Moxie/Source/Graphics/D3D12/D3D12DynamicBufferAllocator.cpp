...
namespace Mox{ 
	...
	D3D12DynamicBufferAllocator::D3D12DynamicBufferAllocator(Mox::D3D12Resource& InResource)
		: m_Resource(InResource), m_TotalAllocationSize(InResource.GetSize()) // At the moment we are using the entire resource for the linear buffer allocator
	{
		...
		m_AllocatedBufferResources.reserve(500);
	}
	...
	Mox::BufferResource& D3D12DynamicBufferAllocator::Allocate(uint32_t InSize)
	{
		...
		m_AllocatedBufferResources.emplace_back( std::make_unique<Mox::D3D12BufferResource>(
			Mox::RES_CONTENT_TYPE::CONSTANT, Mox::BUFFER_ALLOC_TYPE::DYNAMIC, m_Resource, cpuPtr, gpuPtr, alignedSize) );

		return *m_AllocatedBufferResources.back().get();
	}

	void D3D12DynamicBufferAllocator::OnFrameStarted()
	{
		...
		for (auto& currrentBuffer : m_AllocatedBufferResources)
		{
			void* cpuPtr;
	...
	void D3D12DynamicBufferAllocator::AllocateMemoryForBuffer(uint32_t InSizeBytes, void*& OutCpuPtr, D3D12_GPU_VIRTUAL_ADDRESS& OutGpuPtr)
	{
	...
	void D3D12DynamicBufferAllocator::Reset()
	{
