...
namespace Mox
{
	...
	void D3D12VertexBufferView::ReferenceResource(Mox::Resource& InResource, size_t DataSize, size_t StrideSize)
	{
	...
	void D3D12IndexBufferView::ReferenceResource(Mox::Resource& InResource, size_t InDataSize, Mox::BUFFER_FORMAT InFormat)
	{
	...
	void D3D12ShaderResourceView::InitAsTex2DOrCubemap(Mox::Texture& InTexture)
{
		if (!m_CpuAllocatedRange)
		{
			...
			m_CpuAllocatedRange = static_cast<Mox::D3D12GraphicsAllocator*>(Mox::GraphicsAllocator::Get())->GetCpuHeap().AllocateStaticRange(1);
		}
	...
	void D3D12UnorderedAccessView::InitAsTex2DArray(Mox::Texture& InTexture, uint32_t InArraySize, uint32_t InMipSlice, uint32_t InFirstArraySlice, uint32_t InPlaneSlice)
	{
		// Allocate static descriptor in the CPU-only desc heap
		if (!m_CpuAllocatedRange)
		{
	...
	D3D12ConstantBufferView::D3D12ConstantBufferView(Mox::Buffer& InResource)
	{
		Mox::D3D12Resource& buffer = static_cast<Mox::D3D12Resource&>(InResource);
	...
	void D3D12ConstantBufferView::ReferenceBuffer(D3D12_GPU_VIRTUAL_ADDRESS InBufferGPUAddress, size_t InBufferSize)
	{
		if (!m_CpuAllocatedRange)
		{
			...
			m_CpuAllocatedRange = static_cast<Mox::D3D12GraphicsAllocator*>(Mox::GraphicsAllocator::Get())->GetCpuHeap().AllocateStaticRange(1); // Even if we have a dynamic buffer, the descriptor on the CPU staging desc heap will be allocated statically, and change value frequently
		}
	...
	void D3D12ConstantBufferView::ReferenceBuffer(Mox::Buffer& InResource, size_t InDataSize, size_t InStrideSize)
	{
	...
	void D3D12DynamicBuffer::SetData(void* InData, size_t InSize, size_t InAlignmentSize)
	{
		...
		m_AlignmentSize = Mox::Align(InAlignmentSize, D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);

		m_DataSize = InSize;
		
		m_BufferSize = Mox::Align(InSize, m_AlignmentSize);
	...
	D3D12Texture::D3D12Texture(uint32_t InWidth, uint32_t InHeight, Mox::TEXTURE_TYPE InType, Mox::BUFFER_FORMAT InFormat, uint32_t InArraySize, uint32_t InMipLevels)
	{
		SetGeneralTextureParams(InWidth, InHeight, InType, InFormat, InArraySize, InMipLevels, Mox::RESOURCE_FLAGS::NONE);
	}

	D3D12Texture::D3D12Texture(const wchar_t* InTexturePath, Mox::TEXTURE_FILE_FORMAT InFileFormat, int32_t InMipsNum, Mox::RESOURCE_FLAGS InCreationFlags)
	{
		...
		switch (InFileFormat)
		{
		case Mox::TEXTURE_FILE_FORMAT::DDS:
		{
			Mox::ThrowIfFailed(DirectX::LoadFromDDSFile(
				InTexturePath,
		...
		SetGeneralTextureParams(metadata.width, metadata.height, metadata.IsCubemap() ? Mox::TEXTURE_TYPE::TEX_CUBE : Mox::TextureTypeToEngine(metadata.dimension), 
			Mox::BufferFormatToEngine(metadata.format), metadata.arraySize, InMipsNum == 0? metadata.mipLevels : InMipsNum, InCreationFlags);
	...
	void D3D12Texture::UploadToGPU(Mox::CommandList& InCommandList, Mox::Buffer& InIntermediateBuffer)
	{
		...
		::UpdateSubresources(d3d12CmdList, m_D3D12Resource.Get(), static_cast<Mox::D3D12Resource&>(InIntermediateBuffer).GetInner().Get(), 0, 0, m_SubresourceDesc.size(), m_SubresourceDesc.data());
